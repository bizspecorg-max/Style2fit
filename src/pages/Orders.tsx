...
// ----------------------------------------------------------------------
// Unique order code generator (date + random numbers)
// ----------------------------------------------------------------------
...
const Orders = () => {
	...
	const loadAll = async () => {
		...
		const [oRes, cRes, sRes] = await Promise.all([
			supabase
				.from("orders")
				.select(
					"id, code, status, payment_status, delivery_date, price, notes, measurement_values, customer_id, style_id, customers(name, phone), styles(name, image_url)"
				)
	...
	// Open modal from URL param ?new=1&style=ID
	useEffect(() => {
		if (params.get("new") === "1") {
	...
	// ----------------------------------------------------------------------
	// Save order with unique code generation and retry on duplicate
	// ----------------------------------------------------------------------
	const save = async () => {
		...
		try {
			...
			if (createNew) {
				...
				if (!np.success) {
					toast.error(np.error.issues[0].message);
					setSaving(false);
					return;
				}
			...
			// Retry logic for unique order code (up to 3 attempts)
			let attempts = 0;
			const maxAttempts = 3;
			let inserted = false;
			...
			if (!inserted) {
				throw lastError instanceof Error
					? lastError
					: new Error("Failed to generate unique order code");
			}
							...
							<div className="flex items-start gap-3">
								...
								<div className="min-w-0 flex-1">
									<div className="flex items-center gap-2 flex-wrap">
										<span className="font-mono text-xs px-1.5 py-0.5 rounded bg-accent/15 text-accent-foreground border border-accent/30">
											{o.code}
										</span>
									...
									<div className="flex flex-wrap items-center gap-2 mt-3">
										...
										<Badge variant="outline" className="text-[10px]">
											{Object.keys(o.measurement_values ?? {}).length}{" "}
											measurements
										</Badge>
										<Button
											size="icon"
