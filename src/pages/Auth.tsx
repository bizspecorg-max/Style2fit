...
function SignInForm({ onForgot }: { onForgot: () => void }) {
	...
	return (
		<form onSubmit={submit} noValidate className="space-y-5">
			...
			<Button type="submit" className="h-12 w-full rounded-full text-base shadow-soft" disabled={submitting}>
					...
					<li key={label} className="flex flex-1 items-center gap-2" aria-current={step === n ? "step" : undefined}>
						<span
							className={cn(
								...
								step === n && "border-primary text-primary ring-4 ring-primary/10",
						...
						{i === 0 && <span className={cn("h-0.5 flex-1 rounded-full", done ? "bg-primary" : "bg-border")} aria-hidden />}
					</li>
				);
			})}
		</ol>
		...
		<div className="space-y-6">
			...
			{step === 1 ? (
				<form onSubmit={next} noValidate className="space-y-5">
					<div>
						<h2 className="font-display text-3xl leading-tight">{t("signup.accountTitle")}</h2>
					...
					<FormError message={errors.form} />
					<Button type="submit" className="h-12 w-full rounded-full text-base shadow-soft">
						{t("signup.continue")}
					</Button>
				</form>
			) : (
				<form onSubmit={create} noValidate className="space-y-5">
					<div>
						<h2 className="font-display text-3xl leading-tight">{t("signup.shopTitle")}</h2>
					...
					<div className="flex gap-2">
						<Button type="button" variant="outline" className="h-12 rounded-full px-5" onClick={() => setStep(1)} disabled={submitting}>
							<ArrowLeft className="h-4 w-4" />
							{t("signup.back")}
						</Button>
						<Button type="submit" className="h-12 flex-1 rounded-full text-base shadow-soft" disabled={submitting}>
							{submitting && <Loader2 className="h-4 w-4 animate-spin" />}
...
const Auth = () => {
	...
	const points = [t("brand.points.one"), t("brand.points.two"), t("brand.points.three")];
	...
	const homeLink = "rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e7c77e]";
	...
	return (
		<div className="grid min-h-dvh bg-background lg:grid-cols-[1.05fr_1fr]">
			...
			<aside className="relative hidden flex-col justify-between overflow-hidden bg-gradient-hero p-12 text-primary-foreground lg:flex">
				<div className="pointer-events-none absolute -right-24 -top-24 h-96 w-96 rounded-full bg-accent/10 blur-3xl" aria-hidden />
				<Link to="/welcome" className={cn(homeLink, "self-start")}>
					<Logo on="dark" size="lg" />
				</Link>
				<div className="relative max-w-md">
					<p className="eyebrow text-[#e7c77e]">{t("landing.eyebrow")}</p>
					<h1 className="mt-4 font-display text-5xl leading-[1.05] text-balance">{t("brand.tagline")}</h1>
					<p className="mt-5 text-lg leading-relaxed text-primary-foreground/75">{t("brand.pitch")}</p>
					<ul className="mt-8 space-y-3 text-sm">
						{points.map((p) => (
							<li key={p} className="flex items-center gap-3">
								<span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/10" aria-hidden>
									<Check className="h-3.5 w-3.5 text-[#e7c77e]" />
								</span>
								{p}
							</li>
						))}
				...
				<p className="relative text-xs text-primary-foreground/55">{t("landing.trust")}</p>
			</aside>

			<main className="flex flex-col">
				{/* Brand header — phone & tablet */}
				<div className="bg-gradient-hero px-5 pb-12 pt-5 text-primary-foreground lg:hidden">
					<Link to="/welcome" className={cn(homeLink, "inline-flex")}>
						<Logo on="dark" size="sm" />
					</Link>
					<p className="mt-7 font-display text-3xl leading-tight text-balance">{t("brand.tagline")}</p>
					<p className="mt-2 text-sm text-primary-foreground/70">{t("landing.trust")}</p>
				</div>

				<div className="-mt-6 flex flex-1 justify-center rounded-t-[1.75rem] bg-background px-5 pb-10 pt-7 sm:px-8 lg:mt-0 lg:items-center lg:rounded-none">
					<div className="w-full max-w-md">
						<Tabs value={tab} onValueChange={(v) => setTab(v as "signin" | "signup")}>
							<TabsList className="mb-8 grid h-12 w-full grid-cols-2 rounded-full bg-muted p-1">
								<TabsTrigger value="signin" className="h-10 rounded-full text-sm data-[state=active]:shadow-card">
									{t("auth.signInTab")}
								</TabsTrigger>
								<TabsTrigger value="signup" className="h-10 rounded-full text-sm data-[state=active]:shadow-card">
									{t("auth.signUpTab")}
								</TabsTrigger>
							</TabsList>
							...
							<TabsContent value="signin" className="space-y-6">
								<div>
									<h2 className="font-display text-3xl leading-tight">{t("auth.welcomeBack")}</h2>
