...
import { LayoutDashboard, Users, Shirt, ClipboardList, LogOut, Scissors } from "lucide-react";
    ...
    <div className="min-h-screen bg-background pb-20 md:pb-0 md:pl-64">
      ...
      <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-64 flex-col bg-sidebar text-sidebar-foreground border-r border-sidebar-border">
        <div className="px-6 py-6 flex items-center gap-2">
          ...
          <div>
            <div className="font-display text-xl font-bold tracking-tight">Style2Fit</div>
            <div className="text-[11px] uppercase tracking-widest text-sidebar-foreground/60">Tailor OS</div>
          </div>
        </div>
        ...
        <div className="p-3">
          <Button variant="ghost" onClick={handleSignOut} className="w-full justify-start text-sidebar-foreground/80 hover:bg-sidebar-accent hover:text-sidebar-foreground">
          ...
          {nav.map((n) => (
            <li key={n.to}>
              ...
              >
                {({ isActive }) => (
                  <>
                    <n.icon className={cn("h-5 w-5", isActive && "stroke-[2.5]")} />
                    {n.label}
                  </>
                )}
              </NavLink>
